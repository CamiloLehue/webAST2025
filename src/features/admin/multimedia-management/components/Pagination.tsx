...
const PaginationComponent: React.FC<PaginationProps> = ({
  ...
  return (
    <div className="flex items-center justify-between border-t border-bg-300 bg-bg-100 px-4 py-3 sm:px-6 rounded-full">
      <button
        onClick={() => onPageChange(currentPage - 1)}
        disabled={!hasPrevPage || loading}
        className="relative inline-flex items-center rounded-full border border-bg-300 bg-bg-100 px-4 py-2 text-sm font-medium text-white-100 hover:bg-bg-400 disabled:bg-bg-400 disabled:text-white/50 disabled:cursor-not-allowed"
      >
        Anterior
      </button>

      <div className=" sm:flex sm:flex-1 sm:items-center sm:justify-center gap-5">
        <div>
          <nav
      ...
      <button
        onClick={() => onPageChange(currentPage + 1)}
        disabled={!hasNextPage || loading}
        className="relative ml-3 inline-flex items-center rounded-full border border-bg-300 bg-bg-100 px-4 py-2 text-sm font-medium text-white-100 hover:bg-bg-400 disabled:bg-bg-400 disabled:text-white/50 disabled:cursor-not-allowed"
      >
        Siguiente
      </button>
    </div>
